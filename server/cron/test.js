...
let pushedDefault = false;
  ...
  while (currentAccountCount < count) {
	...
	if(!pushedDefault)
	{
		...
		currentFrontiers.push("paw_1k4aesng5ch4swoi91jmi1afzki8nz8jmgpkrriskndren36zcdu4p9iecb6");
		currentFrontiers.push("paw_1ngb95ehimo41985f6zgfqs8nytc4u6qzrb4qrtijsp1jsjrsnuzmfeiaym6");
		currentFrontiers.push("paw_1qj9k17c69r9rmtjp4kw5dmxedaef6xqk6g6gp9zam98jbfso365wif88t16");
		currentFrontiers.push("paw_3ns1ihdrdmtys65xsntgohdu78bu7heuzkb7hpcn179q6k9qi7rg336scbit");
		currentFrontiers.push("paw_3xb4y7j4ojr4ebcbcsfj6op6fj65pauik7rircuishggefh6rmtr7f1p6n9s");
		currentFrontiers.push("paw_1dipnq1qw48nihx1hjyai1ikmk33qhn5n8spciyxq89ngck4jac5pu6g11ib");
		currentFrontiers.push("paw_148ou6ggnbngkhd5nz4oe9rhfdj6wofzs3nh39xw8jrgzz6o79zgtzjtho61");
		currentFrontiers.push("paw_339miuiywqymx6d3mqcjekgf9a4r6pycyzjmgg1nj6jwwyomawsittgf6d9d");
		currentFrontiers.push("paw_1hhqkahbwum3to37pqxqifdz7p5qbpjpysitpt1bmgo15jjyzuoh635j51pt");
		currentFrontiers.push("paw_1m1sozq9irc9w9cycwi8yok7i7kwky5tc9doathtz1cqxj8ec5kskhemj6im");
		currentFrontiers.push("paw_1qfe5u7bcm7qrpp9rhk9p7wyqw316om1ts7s4gm466nwy6ueniik1gzwcno8");
		currentFrontiers.push("paw_35ggj5pisnaa4gt3px8y45ex33978ao41nayofpf8cjwxj6oyq791gqxcgmm");
		currentFrontiers.push("paw_3xc9sqacbec1mfku1mebsd1g8kuyp1aa8wqoakg1t51ch4hze35wpni9grhj");
...
const getDistribution = async () => {
  ...
  for (let y = 0; y < tmpAccountFiles.length; y++) {
	...
	for (let i = 0; i < balancesChunks.length; i++) {
      ...
      await Promise.all(
        Object.entries(balances).map(
          async ([account, { balance: rawBalance, pending: rawPending }]) => {
			...
			//let entry = {};
			//entry[account] = total;
			//redisClient.zadd(`${REDIS_RICH_LIST}_TMP`, account, total);
			...
			{
				var dist_balance = new BigNumber(total)
	  ...
	  let i = 0;
		for (const [key, value] of Object.entries(richList)) {
			i+=1;
			if(i > 10000)
				break;
		 redisClient.zadd(`${REDIS_RICH_LIST}_TMP`, key, value);
		}

	
    //const richListChunks = chunk(richList, 3);
	//for (let i = 0; i < richListChunks.length; i++) {
		// too much data ( stack size error)
	//	redisClient.zadd(`${REDIS_RICH_LIST}_TMP`, ...richListChunks[i]);
	//}
